...
-- MAGIC 1. The **Single user access** setting allows us to designate who can attach to this cluster. This setting is independent and unrelated to cluster access controls, which existing Databricks users might be familiar with. This setting is enforced on the cluster, and only the user designated here will be able to attach; all others will be rejected. By default this is set to ourselves, which we will leave as is. But consider changing this setting when one of the following apply:
...
-- MAGIC ### Browsing data
-- MAGIC
-- MAGIC Let's take a few moments to get acquainted with the updated **Data** page in the Data Science and Engineering Workspace.
-- MAGIC
...
-- MAGIC ### Browsing data
-- MAGIC
-- MAGIC Let's take a few moments to get acquainted with the updated **Data Explorer** page.
-- MAGIC
