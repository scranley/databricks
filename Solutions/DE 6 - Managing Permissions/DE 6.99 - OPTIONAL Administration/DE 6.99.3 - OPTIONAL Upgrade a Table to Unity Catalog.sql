...
-- MAGIC ## Set Up
-- MAGIC
-- MAGIC Run the following cells to perform some setup. In order to avoid conflicts in a shared training environment, this will create a uniquely named database exclusively for your use. This will also create an example source table called **movies** within the legacy Hive metastore.
...
-- MAGIC %md
-- MAGIC This gives an error since this table lives in the legacy metastore and we are not running on a cluster with legacy table access control enabled. This highlights a key benefit of Unity Catalog: no additional configuration is needed to achieve a secure solution. Unity Catalog is secure by default.
